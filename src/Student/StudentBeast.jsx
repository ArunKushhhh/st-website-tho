  ...
  return (
    <div
      ...
      style={{
        ...
        zIndex: isHovered ? 1000 : 1, // Fixed z-index issue
      }}
          ...
          {isHovered && (
            <motion.div
              ...
              style={{
                ...
                transform: "scaleX(-1)",
                zIndex: 1001, // Ensure overlay is above everything
              }}
            >
              <div
...
const CurvedCarousel = ({ products = SAMPLE_PRODUCTS }) => {
  ...
  const handleAutoRotate = useCallback(() => {
    ...
    if (activeIndex !== null) {
      setActiveIndex(null); // shrink expanded card on auto-rotate
    }
  }, [activeIndex]);

  // Determine if auto-rotation should be active
  ...
  const handleDragMove = useCallback(
    (clientX) => {
      ...
      if (activeIndex !== null) {
        setActiveIndex(null); // shrink expanded card on drag
      }
    },
  ...
  const handleDragEnd = useCallback(() => {
    setIsDragging(false);
    // Auto-rotation will resume automatically via shouldAutoRotate logic
  }, []);
...
const StudentBeast = () => {
  ...
  // 1️⃣ Header + Carousel Animations
  useEffect(() => {
    const elements = {
    ...
    if (isMobile) {
      // Instantly show everything for mobile
  ...
  // 2️⃣ Continuous Initial <-> Final Text Swap
  useEffect(() => {
    const elements = {
    ...
    // Reset visibility (keep both in layout)
    ...
    const trigger = ScrollTrigger.create({
      trigger: elements.container,
      start: "top 70%", // start when section is visible
      once: true,
      onEnter: () => {
        tl = gsap.timeline({ repeat: -1, repeatDelay: 0 });

        // Step 1: Initial visible
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          ref={carouselRef}
          className="relative flex items-center justify-center mt-2 px-4 md:px-4"
        >
