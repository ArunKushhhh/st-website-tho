...
function Home() {
  ...
  return (
    <div className="h-screen flex flex-col sm:flex-row relative overflow-hidden">
      
        {/* Left Column */}
        <div 
          ref={leftColumnRef}
          className="flex w-full sm:w-[50%] relative h-[60%]  sm:min-h-screen "
          style={{ backgroundImage: `url(${ourofferings1})` }}
        >
          {/* Left Content */}
          <div className="flex flex-col justify-center pt-20 sm:pt-0 sm:pb-0 sm:justify-center items-start p-8 lg:px-16 bg-[#3D070D] opacity-80">
  ...
  <p className="text-[clamp(12px,calc(12px+(18-12)*(100vh-600px)/(900-600)),18px)] sm:text-base lg:text-lg xl:text-xl text-white/90 sm:mb-8 leading-relaxed drop-shadow-lg text-left">
    Whether you're targeting one neighborhood or the entire country, we build campaigns that grow with you — meaningfully and measurably.
        ...
        <div 
          ref={rightColumnRef}
          className="flex w-full sm:w-[50%] relative h-[40%]  sm:min-h-screen"
          style={{
        ...
        >
          ...
          <div className="flex flex-col sm:pt-8 justify-center items-end px-8 lg:px-16  bg-[#FFD2D7] opacity-80">
            {/* Main Content */}
            <button className="bg-[#b8001f] hover:bg-[#9a0019] text-sm sm:text-base sm:mb-8 text-white font-semibold px-4 py-3 sm:px-8 sm:py-4 rounded-full transition-all duration-300 hover:scale-105 shadow-2xl">
              Explore More →
            </button>
